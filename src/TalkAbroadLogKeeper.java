...
//                                                                appointmentControlPanel.add(showDescription);                                                               
	...
	private void checkSkypeRunning(){
		...
		try{
			while (!monitor.isRunning()){
				...
				if (!monitor.isRunning()) displaySkypeNotRunning();
				count = 0;
			}
		} catch (Exception e){
			e.printStackTrace();
 ...
 ////////////////////////////////////////////////////////////////////   Partho, you need to check this method         
	 ...
	 * If the user is logged in, first display a modal dialog to
	 * confirm if they want to quit.
	 */
			...
			if (n == JOptionPane.YES_OPTION){
				...
				if (!list.isEmpty()){
					...
					if (n == JOptionPane.YES_OPTION){
						...
						while (itr.hasNext()) itr.next().delete();
...
////////////////////////////////////////////////////////////////////   Partho, you need to check this method  		
	 ...
	 * and switches display of the top and main panels by removing the
	 * splash screen and replacing the login interface with a banner.
	 */
...
//                        System.out.println(selectedAppointment.getDescription());                        
	 ...
	 * This is a utility function to aid with building file names for recording files.
	 * Example: "recordings/12345-"
	...
	/**
	 * Build the appointee recording file based on the selected appointment and the recording iteration.
	 * This is a utility function to aid with loading recordings for multi-session conversations.
	...
	/**
	 * Build the mic recording file based on the selected appointment and the recording iteration.
	 * This is a utility function to aid with loading recordings for multi-session conversations.
	 * @param session The number of the session
	 * @return A file object representing the recording.
	 */
	...
	/**
	 * Obtain the next session number for the selected appointment based on the current size of the
	 * recording file vectors appointee_recordings and mic_recordings.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns a File object that represents the final mixdown MP3 of the conversation.
	...
	/**
	 * Utility method to parse the integer appointment ID from the name of a recording file.
	 * @param in : an audio recording file associated with an appointment
	 * @return the integer id of the appointment associated with this recording file
	 */
	...
	/**
	 * Returns an ArrayList with recording files no longer necessary. 
	 * Only audio files associated with appontments that are not nece
	 * @param in_files
	 * 
	 */
...
//                        clickLogInButton();
	...
	public void uploadConversation(){
		...
		/**
		 * This internal class is a SwingWorker class that processes the mixing of the
		 * recorded audio files in the background while interacting with
		...
		class ConversionTask extends SwingWorker<Integer,Integer>{
			...
			public Integer doInBackground(){
		    	...
		    	/**
		    	 * This internal class implements the runnable interface and is spawned
		    	 * by the SwingWorker thread so that the actual writing of the
		    	 * audio file is in its own isolated thread. This permits monitoring
		    	 * of how many bytes have been mixed down within the SwingWorker thread
		    	 * and updating the UI via the JLabel object passed to the SwingWorker's
		    	 * constructor.
		    	 * @author Masahiro Knittel
		    	 *
		    	 */
		...
		/**
		 * This internal class is a SwingWorker class that processes the uploading of the
		 * recorded audio files in the background while interacting with
		 * a JLabel to notify the user of how many bytes have been written.
		 * @author Masahiro Knittel
		 *
		...
		class UploadTask extends SwingWorker<Integer,Integer>{
			...
			public Integer doInBackground(){
				...
				/**
				 * This internal class implements the runnable interface and is spawned
		    	 * by the SwingWorker thread so that the actual transmission of the
		    	 * audio file is in its own isolated thread. This permits monitoring
		    	 * of how many bytes have been sent to the server by the SwingWorker thread
		    	 * and updating the UI via the JLabel object passed to the SwingWorker's
		    	 * constructor.
				 * @author Masahiro Knittel
				 *
	...
	/**
	 * Overrides the JFrame's default paint method.
	 * This method first checks to see if the user is logged in, and if not,
	 * displays the splash image in the main panel. If the user is logged in,
	 * it instead displays the banner image in the top panel where the log in
	 * interface was.
