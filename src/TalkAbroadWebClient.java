...
/**
 * Client object that talks to the TalkAbroad server. This is used by
 * the main app to send and receive communications with the server.
 * RSA public key encryption is used to authenticate users with the specified
 * email and password, as well AES symmetric encryption for sensitive data
 * transmissions.
 * 
...
public class TalkAbroadWebClient {
	...
	/**
	 * Constructs a client that talks to the TalkAbroad server. 
	 * The public key is obtained from the server via HTTP request.
	 * The AES key and IV is initialized and sent with the security token
	 * for every request that is sent to the server for symmetric communication.
	 * 
...
//        I have to work with the following 4 functions to change the STATUS functionality
	...
	/**
	 * Sends a request to the TalkAbroadLogKeeper server. The only parameter required
	 * to have actual data is the address. The data parameter contains a key:value hash
	 * table equivalent to an associative array sent to the server for POST request data.
	 * 
	 * This is an overloaded method for simpler requests that require no file uploads.
	 * 
	 * @param address URL of the script on the server
	 ...
	 * table equivalent to an associative array sent to the server for POST request data.
	 * 
	 * This is an overloaded method that automates uploads of files larger than max_upload_size
	 * by making multiple send_request calls to the server.
	 * 
	 * @param address URL of the script on the server
	 * @param data a hashtable of strings containing key:value pairs
	 ...
	 * to have actual data is the address. The data parameter contains a key:value hash
	 * table equivalent to an associative array sent to the server for POST request data.
	 * File uploads are accomplished by sending chunks of data up to a maximum upload size
	 * as set in the private class property max_upload_size. For files larger than this size,
	 * multiple calls to this method are necessary. This is automated by calling this method
	 * with the overloaded method with the (String, Hashtable, File) signature. This method
	 * is private and as such, the overloaded methods must be used instead.
	 * 
	 * @param address URL of the script on the server
	 * @param data a hashtable of strings containing key:value pairs
	...
	private String send_request(String address, Hashtable<String,String> data, String filename, byte[] chunk, int length){
		...
		try{
			if (data == null) data = new Hashtable<String,String>();
